...
const Displayrides = () => {
  ...
  const checkActiveRide = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/v1/driver/getactiveride", {
  ...
  const fetchrides = async () => {
    try {
      ...
      const res = await axios.get("http://localhost:5000/api/v1/showallrides", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const skipride = async (rideid) => {
    try {
      const token = localStorage.getItem("driverToken");
      const res = await axios.post(
        "http://localhost:5000/api/v1/skipride",
        { rideid },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const completeRide = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("driverToken");
      const res = await axios.post(
        "http://localhost:5000/api/v1/completeride",
  ...
  const cancelRide = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("driverToken");
      const res = await axios.post(
        "http://localhost:5000/api/v1/driver/cancelride",
        { rideid: acceptedRide._id },
        { headers: { Authorization: `Bearer ${token}` } }
      );
